...
+ (BOOL)runProcessAsAdministrator:(NSString *)scriptPath
    ...
    NSString *fullScriptCommand =
        [NSString stringWithFormat:@"%@ %@", scriptPath, allArgs];

    // Escape double quotes and backslashes for the AppleScript
    fullScriptCommand =
        [fullScriptCommand stringByReplacingOccurrencesOfString:@"\""
                                                     withString:@"\\\""];
    fullScriptCommand =
        [fullScriptCommand stringByReplacingOccurrencesOfString:@"\\"
                                                     withString:@"\\\\"];

    // Create the AppleScript that runs the shell command with administrator
    // privileges
    ...
    @try {
        ...
        *errorDescription = [NSString
            stringWithFormat:@"Failed to launch process: %@", exception.reason];
        return NO;
    }
...
+ (BOOL)runProcessAsAdministratorPipeOutput:(NSString *)scriptPath
    ...
    NSString *appleScriptSource = [NSString
        stringWithFormat:
            @"do shell script \"%@ > %@\" with administrator privileges",
            fullScriptCommand, outputPipe.path];
...
+ (void)executeAdminProcessWithPipe:(NSString *)scriptPath
    ...
    dispatch_async(
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          ...
          BOOL success =
              [self runProcessAsAdministratorPipeOutput:scriptPath
